ActiveRecord::ConnectionAdapters::PostgreSQLAdapter.class_eval do
  # Creates aggregate updateable view of parent and child relations. The convention uses for naming child tables is
  # <tt>child_view</tt> with <tt>_data</tt> appended. If you don't follow the convention, you must supply the child
  # table name as third argument.
  ...
  # Returns a relation's primary key and belonging sequence. If the relation is a table the result is its pk and sequence,
  # when it is a view, those of the table at the root of the inheritance chain.
  ...
  # Creates Single_Table_Inheritanche-like aggregate view called +sti_aggregate_view+
  ...
  # by passing the columns' names in the desired order in the +columns_for_view+ argument.
  #
  # Note that if there are conflict columns with the same name but different types in in two or more relations 
  # these columns are merged into one of type 'text' into the aggregate view. 
  # Also note that this method uses reflection on the Rails application's models and their tables
  # where there can be no more than one model class for a table.
  #
  # The method uses class name stored by Rails' SingleTableInheritance mechanism in the inheritance column of the +parent_relation+
  # to detirmine which tuples (records) in parent_relation and its descendants relations are actually a part of a record for 
