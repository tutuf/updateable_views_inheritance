    ...
    module PostgreSQL
      module SchemaStatements
          ...
          class ClassTableReflection
            class << self
              ...
              def all_db_klasses
                ...
                model_filenames.collect { |m|
                  load "#{Rails.root}/app/models/#{m}"
                  m.match(%r{([^/]+?)\.rb$})[1].camelize.constantize
                }.each do |klass|
        ...
        def get_leaves_relations(hierarchy)
          ...
          if head.is_a? Array
            (get_leaves_relations(head) + get_leaves_relations(hierarchy)).compact
          elsif hierarchy.nil? || hierarchy.empty?
            [head]
          else
            get_leaves_relations(hierarchy).compact
          end
        end
