...
in the inheritance chain. The approach was first suggested by John Wilger (http://johnwilger.com/).

API is available at http://clti.rubyforge.org/

==Supported DBMS

Currently only PostgreSQL is supported. Adding another DBMS (should support updatable aggregate views) is as easy as writing an ActiveRecord database adapter extension.
