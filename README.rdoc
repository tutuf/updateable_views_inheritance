...
Currently only PostgreSQL is supported, but it should be easy to add other DBMSes
that have updatable aggregate views.
...
Run rake class_table_inheritance:fixture to generate fixture for the +class_table_inheritance+ table. <b>Without it primary key sequence for 
...
==TODO
- Utility method (or better, a trigger) to fix possible inconsistency when children and parent records in class_table_inheritance table no longer exist as tables.
