...
Store attributes of every inheritor class in its own table. More about the pattern on http://www.martinfowler.com/eaaCatalog/classTableInheritance.html.
...
This plugin re-enables referential integrity on fixture loading, so if you are using foreign keys you will have to explicitly declare fixture load order in test_helper.rb like this:
...
Rails: 2.1+ (tested with 2.1.2, 2.2.2 and 2.3.3, might work with 2.0.x)

Ruby: tested with 1.8.6 and 1.9.1 

Database: PostgreSQL 8.1+ only. Patches for other DBMS are welcome. Note for developers: it's not required to use updateable views - children relations can be tables (with some triggers involved) or materialized views.
